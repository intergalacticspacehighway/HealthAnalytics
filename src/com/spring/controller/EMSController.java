...
public class EMSController {
	...
	@MessageMapping("/updateLoc")
	public void updateLoc(Principal principal, EMSRequestVO request)
			throws Exception {
		EMSVO ems = new EMSVO();
		ems.setLatitude(request.getLatitude());
		ems.setLongitude(request.getLongitude());
		ems.setUserName(principal.getName());
		insertEMSDao.insertHospital(ems);
		
		System.out.println("Hello:: FromUpdateLoc");
		System.out.println(request.getLatitude());
	...
	public ModelAndView addEMS() throws Exception {
		return new ModelAndView("admin/addEMS", "insertEMS",
				new EMSRegistrationVO());
	}
